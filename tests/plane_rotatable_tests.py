...
class PlaneRotatableTests:
    ...
    implementing the methods `construction_test`, `rotate_cw_test`, `rotate_ccw_test`,
    and `rotate_ht_test`.

    The design is such that once each of the rotation methods are sufficiently tested
    for some ground case(s) in `rotate_*_test`, then the property based tests implemented
    ...
    `rotate_*_test` should be as straightforward and hard coded as possible. Only a few
    ...
    that a subset of the many possible rotatable objects are hard coded into the strategy
    definitions.
    """
